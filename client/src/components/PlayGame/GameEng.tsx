...
function GameEng() {
  ...
  const [{ date: savedDate, guesses, winState, letterStates }, setSavedGame] =
    useLocalStorage(SAVED_GAME_KEY, SAVED_GAME_INIT);
  ...
  const updateStatistics = (isGameWon: boolean, guessesAmount: number) => {
    ...
    const guessResult = (
      isGameWon ? guessesAmount.toString() : "X"
    ...
    setStatistics({
      ...
      maxStreak:
        newStreak > statistics.maxStreak ? newStreak : statistics.maxStreak,
    });
  };
  ...
  const updateLetterState = (
    states: KeyboardLetterStates,
    letter: string,
    newState: GuessLetterState
  ) => {
    if (states[letter]) {
  ...
  const validateLastGuess = (): GuessValidationResult => {
    ...
    for (let i = 0; i < WORD_SIZE; i++) {
      const letterState =
        lastGuess[i].letter === dailyWordLetters[i] ? "right" : "wrong";
    ...
    if (missingLetters.length) {
      const wrongLetters = validatedGuess.filter(
        (guess) => guess.state === "wrong"
      );

      for (let guessLetter of wrongLetters) {
        const indexOnMissingLetters = missingLetters.indexOf(
          guessLetter.letter
        );
  ...
  const handleKeyboardEnter = () => {
    ...
    if (!isLastGuessInWordList()) {
      const newGuess = getLast(guesses).map(
        (guess) =>
          ({ letter: guess.letter, state: "wordlistError" } as GuessLetter)
      );
  ...
  return (
    <div
      className="mt-3"
      onClick={
        winState.isGameEnded && !isEndGameScreenOpen
          ? () => setIsEndGameScreenOpen(true)
          : undefined
      }
      style={{
